...
export class ClientService {
    ...
    async getAll(query: ApiQuery) {
      try {
        ...
        const response = await httpClient.get<{ data: Client[], count: number }>("clients", {
          params: {
            $skip: query.pageIndex * query.pageSize,
            $top: query.pageSize,
            $filter: `substringof('${normalizedQueryString}', tolower(email)) or substringof('${normalizedQueryString}', tolower(name))`,
            $select: "id, name, lastName, maritalStatus, email" },
        });
        return response.data;
      } catch (error) {
        return null;
